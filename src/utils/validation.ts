...
type FailMap = Record<string, { count: number; lockedUntil?: number }>

function readFailures(): FailMap {
  try {
    const s = localStorage.getItem(AUTH_FAIL_KEY)
...
function writeFailures(map: FailMap) {
  try { localStorage.setItem(AUTH_FAIL_KEY, JSON.stringify(map)) } catch {}
}

export function canAttemptAuth(identifier: string): boolean {
  const map = readFailures()
  const rec = map[identifier]
  if (!rec) return true
...
export function registerAuthFailure(identifier: string) {
  const map = readFailures()
  const rec = map[identifier] || { count: 0 }
  rec.count += 1
...
export function resetAuthFailures(identifier: string) {
  const map = readFailures()
