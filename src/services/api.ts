...
class ApiService {
  ...
  private async request<T>(
    ...
    const config: RequestInit = {
      headers: {
        ...getAuthHeaders(),
        // En-têtes de cache intelligents
        'Cache-Control': options.method === 'GET' ? 'public, max-age=300, stale-while-revalidate=600' : 'no-cache',
        'X-Request-ID': `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        ...options.headers,
      },
      // Options de cache par défaut
  ...
  async get<T>(endpoint: string, options?: { useCache?: boolean }): Promise<ApiResponse<T>> {
